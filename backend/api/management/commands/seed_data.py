    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username=admin_username).exists():
            ...
            if admin_password:
                ...
                self.stdout.write(f"Superuser '{admin_username}' already exists. Preserving existing password.")

        # 2. Profile Initial Data (NEVER overwrite existing data on deploy/reboot)
        ...
        if created:
            ...
            profile.save()
            self.stdout.write(self.style.SUCCESS("Initial profile created for Petla Siva Kumar."))
        else:
            self.stdout.write("Existing profile found. Preserving all custom profile edits.")
        ...
        if Experience.objects.count() == 0:
            ...
            self.stdout.write(self.style.SUCCESS("Initial experience record created."))
        else:
            self.stdout.write("Existing experience records found. Preserving experience data.")

        self.stdout.write(self.style.SUCCESS("Database check complete. No user data overwritten!"))
